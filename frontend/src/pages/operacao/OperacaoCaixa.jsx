...
export default function OperacaoCaixa() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-display font-semibold text-char mb-6">Caixa do salão</h1>
      ...
      {!sessao ? (
        ...
        <div className="card p-6 max-w-md mb-8">
          ...
          <p className="text-sm text-ink-soft mb-4">
            Fundo de troco: <span className="font-mono text-char">{money(sessao.fundoTroco)}</span>
          </p>
            ...
            <div key={h.id} className="card p-5">
              <div className="flex items-center justify-between flex-wrap gap-2 mb-2">
                <p className="text-sm font-semibold text-char">
                  {new Date(h.abertoEm).toLocaleDateString("pt-BR")} — {STATUS_LABEL[h.status]}
                </p>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm mb-2">
                <p>
                  <span className="text-ink-soft">Dinheiro: </span>
                  <span className="font-mono">{money(h.totalVendasDinheiro)}</span>
                </p>
                <p>
                  <span className="text-ink-soft">Cartão: </span>
                  <span className="font-mono">{money(h.totalVendasCartao)}</span>
                </p>
                <p>
                  <span className="text-ink-soft">PIX: </span>
                  <span className="font-mono">{money(h.totalVendasPix)}</span>
                </p>
                <p>
                  <span className="text-ink-soft">Saldo calculado: </span>
                  <span className="font-mono">{money(h.saldoFinalCalculado)}</span>
                </p>
              </div>
              ...
              {h.status === "CONFERIDO" && (
                <p className="text-sm">
                  ...
                  <span className="font-mono">{money(h.saldoFinalContado)}</span>
                </p>
              )}
